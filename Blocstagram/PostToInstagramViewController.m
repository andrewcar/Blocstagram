...
//  Blocstagram
//
//  Created by Andrew Carvajal on 4/28/15.
...
- (instancetype)initWithImage:(UIImage *)sourceImage {
    ...
    if (self) {
        ...
        self.filterImages = [NSMutableArray arrayWithObject:self.sourceImage];
...
- (void)addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *bloomFilter = [CIFilter filterWithName:@"CIPhotoEffectProcess"];
        
        if (bloomFilter) {
            [bloomFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:bloomFilter.outputImage withFilterTitle:NSLocalizedString(@"Bloom", @"Bloom Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        // assign sourceCIImage to sepia filter
        ...
        // create a whiteSpecks filter
        CIFilter *whiteSpecks = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, randomImage,
                                 @"inputRVector", [CIVector vectorWithX:0.0 Y:0.1 Z:0.0 W:0.0],
                                 @"inputGVector", [CIVector vectorWithX:0.0 Y:0.1 Z:0.0 W:0.0],
                                 @"inputBVector", [CIVector vectorWithX:0.0 Y:0.1 Z:0.0 W:0.0],
                                 @"inputAVector", [CIVector vectorWithX:0.0 Y:0.01 Z:0.0 W:0.0],
                                 @"inputBiasVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0], nil];
        
        // create a darkScratches filter
        CIFilter *darkScratches = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, otherRandomImage,
                                   ...
                                   @"inputBVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0],
                                   @"inputAVector", [CIVector vectorWithX:0.0 Y:1.0 Z:1.0 W:1.0], nil];
        
        // create a minimum component filter
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *motionFilter = [CIFilter filterWithName:@"CIMotionBlur"];
        
        if (motionFilter) {
            [motionFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:motionFilter.outputImage withFilterTitle:NSLocalizedString(@"Motion", @"Motion Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (tonalFilter && sepiaFilter && composite) {
            ...
            [composite setValue:sepiaFilter.outputImage forKey:kCIInputBackgroundImageKey];
            [self addCIImageToCollectionView:composite.outputImage withFilterTitle:NSLocalizedString(@"Color Blend", @"Color Blend Filter")];
        }
    }];
}
