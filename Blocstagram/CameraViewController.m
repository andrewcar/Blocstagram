    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (granted) {
                ...
                if (!input) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:error.localizedDescription message:error.localizedRecoverySuggestion delegate:self cancelButtonTitle:NSLocalizedString(@"OK", @"OK button") otherButtonTitles:nil, nil];
                    [alert show];
                } else {
...
- (NSArray *)horizontalLines {
    if (!_horizontalLines) {
        _horizontalLines = [self newArrayOfFourViews];
    }
...
- (NSArray *)verticalLines {
    if (!_verticalLines) {
        _verticalLines = [self newArrayOfFourViews];
    }
    return _verticalLines;
}

- (NSArray *)newArrayOfFourViews {
...
- (void)cameraButtonPressedOnToolbar:(CameraToolbar *)toolbar {
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        if (imageDataSampleBuffer) {
            ...
            CGRect gridRect = CGRectMake(CGRectGetMinX(leftLine.frame), CGRectGetMinY(topLine.frame), CGRectGetMaxX(rightLine.frame) - CGRectGetMinX(leftLine.frame), CGRectGetMaxY(bottomLine.frame) - CGRectGetMinY(topLine.frame));
